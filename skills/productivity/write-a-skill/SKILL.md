...
- **Search narrow.** Prefer targeted lookups over whole files. For **code** use `ast-grep` (`sg`), never `grep` (see the `ast-grep` skill); keyword search is only for **prose**.
...
1. **Description with triggers.** Third person, ≤1024 chars. First sentence = what it does; then "Use when …" with concrete trigger phrases. It's the *only* thing the agent sees when deciding to load — make it discriminating.
