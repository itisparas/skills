...
| `state:sliced` (on the PRD) · `state:buildable` · `state:building` · `state:review-ready` · `state:blocked` · `state:merge-ready` · `state:human-review-needed` | Pipeline states set by skills (an **untyped** `state:human-review-needed` issue is `ideate`'s "needs triage" case) |
